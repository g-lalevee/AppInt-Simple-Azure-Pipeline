...
The Gitlab CI pipeline provided uses the `integrationcli apply` command to apply the configurations generated by `integrationcli scaffold`.
...
- Go to your GitLab project > Build > Pipelines.
  ...
  ![GitLab CICD Pipeline](./images/monitor.png)

- Click on the status label: you can see all stages running.

  ![GitLab CICD Pipeline Animated](./images/pipeline.png)

- And the end of test stage you can download artifacts. Click on `apickli` stage, then on **Download** button to download the zip file containe apickli results.

  ![GitLab CICD Pipeline artifacts](./images/logs.png)

- For example, the results of integration tests with Apickli (download a zip file, open html content with your browser)

  ![GitLab CICD Pipeline apickli](./images/apickli.png)
